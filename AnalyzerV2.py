...
class AnalyzerGTFS:
    ...
    lat: float = 0
    lon: float = 0
    date_min: datetime = pd.to_datetime("2024-07-01")
    date_max: datetime = pd.to_datetime("2024-08-31")

    def __init__(self, path="TER"):
        self.calendar_dates = pd.read_csv(os.path.join("Data", path, "calendar_dates.txt"))
        self.stop_times = pd.read_csv(os.path.join("Data", path, "stop_times.txt"))[
            ["trip_id", "stop_id", "departure_time"]
        ]
        self.stops = pd.read_csv(os.path.join("Data", path, "stops.txt"))[
            ["stop_id", "stop_name", "stop_lat", "stop_lon", "parent_station"]
        ]
        self.trips = pd.read_csv(os.path.join("Data", path, "trips.txt"))[["service_id", "trip_id", "route_id"]]
    ...
    def trips_stops_proches(self) -> pd.Series:
        stops_proches: pd.DataFrame = self.stops_proches(self.lat, self.lon)
    ...
    def trips_dans_periode(self) -> pd.Series:
        trips_ids: pd.Series = self.trips_stops_proches()
        ...
        services_dans_periode: pd.DataFrame = services[
            (services["date"] >= self.date_min) & (services["date"] <= self.date_max)
        ]
    ...
    def get_set_destinations(self, lat: float, lon: float, date_min: str, date_max: str) -> pd.DataFrame:
        ...
        trips_ids_dans_periode: pd.Series = self.trips_dans_periode()
    ...
    def get_trajets(self, lat_arrivee: float, lon_arrivee: float) -> pd.DataFrame:
        stops_depart: pd.DataFrame = self.stops[
            (self.stops["stop_lat"] > self.lat - MARGE_DISTANCE)
            & (self.stops["stop_lat"] < self.lat + MARGE_DISTANCE)
            & (self.stops["stop_lon"] > self.lon - MARGE_DISTANCE)
            & (self.stops["stop_lon"] < self.lon + MARGE_DISTANCE)
            & self.stops["parent_station"].str.contains("StopArea")
        ]
        stops_arrivee: pd.DataFrame = self.stops[
            (self.stops["stop_lat"] > lat_arrivee - MARGE_DISTANCE / 2)
            & (self.stops["stop_lat"] < lat_arrivee + MARGE_DISTANCE / 2)
            & (self.stops["stop_lon"] > lon_arrivee - MARGE_DISTANCE / 2)
            & (self.stops["stop_lon"] < lon_arrivee + MARGE_DISTANCE / 2)
            & self.stops["parent_station"].str.contains("StopArea")
        ]
        ...
        trajets: pd.DataFrame = trips_et_jours[
            (trips_et_jours["date"] >= self.date_min) & (trips_et_jours["date"] <= self.date_max)
        ]
        trajets = trajets.assign(horaire_depart="", horaire_arrivee="")
        trajets["horaire_depart"] = trajets["departure_time_x"] + trajets["date"]
        trajets["horaire_arrivee"] = trajets["departure_time_y"] + trajets["date"]
        return trajets
