...
class AnalyzerCalendar(Analyzer):
    ...
    def __init__(self, transport_type="FLIXBUS"):
        self.calendar_dates = pd.read_csv(
            os.path.join("Data", transport_type, "calendar_dates.txt")
        )
        self.calendar = pd.read_csv(
            os.path.join("Data", transport_type, "calendar.txt")
        )
        self.stop_times = pd.read_csv(
            os.path.join("Data", transport_type, "stop_times.txt"), low_memory=False
        ...
        if "stop_timezone" in stops.columns:
            ...
            self.stops = stops[["stop_id", "stop_name", "stop_lat", "stop_lon"]].assign(
    ...
    def filter_trips_within_period(
        ...
        relevant_services: pd.DataFrame = self.calendar[
            self.calendar["service_id"].isin(relevant_trips["service_id"])
        ]
        services_within_period: pd.DataFrame = relevant_services[
            (
                (relevant_services["start_date"] >= start_date)
                & (relevant_services["start_date"] <= end_date)
            )
            | (
                (relevant_services["end_date"] >= start_date)
                & (relevant_services["end_date"] <= end_date)
            )
            | (
                (relevant_services["start_date"] <= start_date)
                & (relevant_services["end_date"] >= end_date)
        ...
        services_within_period = services_within_period.assign(
            days_ok_start=np.vectorize(
                lambda x: start_date if (start_date >= x) else x
            )(services_within_period["start_date"]),
            days_ok_end=np.vectorize(lambda x: end_date if (end_date <= x) else x)(
                services_within_period["end_date"]
            ),
        )
        services_within_period["days_ok"] = (
            services_within_period["days_ok_end"]
            - services_within_period["days_ok_start"]
        )
        # Optimisation de la ligne suivante à faire
        services_in_dates = services_within_period[
            services_within_period.apply(self.is_service_in_dates, axis=1)
        ]
    ...
    def is_service_in_dates(self, services: pd.Series) -> bool:
        ...
        while day_offset <= services.days_ok.days:
            current_day_index = (
                services.days_ok_start + pd.Timedelta(days=day_offset)
            ).weekday()
    ...
    def find_destinations_from_location(
        ...
        departure_time: pd.Series = self.unique_departures[
            "departure_time"
        ]  # StopTime trip
        departure_time.index = self.unique_departures["trip_id"]
        column_departure_time = departure_time.loc[
            cities_after_inital_departure["trip_id"]
        ]
        cities_after_inital_departure["city_departure_time"] = (
            column_departure_time.array
        )

        duplicate_destinations_stop_times: pd.DataFrame = cities_after_inital_departure[
            cities_after_inital_departure["departure_time"]
            > cities_after_inital_departure["city_departure_time"]
        ]
        ...
        destinations = destinations[
            ~destinations["stop_id"].isin(self.nearby_stops["stop_id"])
        ]
        return destinations
    ...
    def find_trips_between_locations(
        ...
        departure_stops: pd.DataFrame = self.find_nearby_stops(
            departure_lat, departure_lon, max_distance
        )
        arrival_stops: pd.DataFrame = self.find_nearby_stops(
            arrival_lat, arrival_lon, max_distance
        )
        trips_containing_departure: pd.DataFrame = pd.merge(
            departure_stops, self.stop_times, on="stop_id"
        )
        trips_containing_departure = trips_containing_departure[
            trips_containing_departure["departure_time"] > departure_time
        ]
        trips_containing_departure = trips_containing_departure.drop_duplicates(
            subset="trip_id"
        )
        trips_containing_arrival: pd.DataFrame = pd.merge(
            arrival_stops, self.stop_times, on="stop_id"
        )
        ...
        trips_in_right_direction: pd.DataFrame = trips_containing_both[
            trips_containing_both["departure_time_x"]
            < trips_containing_both["departure_time_y"]
        ]
        relevant_trips: pd.DataFrame = pd.merge(
            trips_in_right_direction, self.trips, on="trip_id"
        )
        trips_in_dates = self.dates_from_trips(relevant_trips, start_date, end_date)
        return trips_in_dates

    def dates_from_trips(
        self, trips: pd.DataFrame, start_date: datetime, end_date: datetime
    ) -> pd.DataFrame:
        start_date = pd.to_datetime(start_date)
        end_date = pd.to_datetime(end_date)
        services: pd.DataFrame = self.calendar[
            self.calendar["service_id"].isin(trips["service_id"])
        ]
        services_within_period: pd.DataFrame = services[
            (
                (services["start_date"] >= start_date)
                & (services["start_date"] <= end_date)
            )
            | (
        ...
        services_within_period = services_within_period.assign(
            days_ok_start=np.vectorize(
                lambda x: start_date if (start_date >= pd.to_datetime(x)) else x
            )(services_within_period["start_date"]),
            days_ok_end=np.vectorize(
                lambda x: end_date if (end_date <= pd.to_datetime(x)) else x
            )(services_within_period["end_date"]),
        )
        ...
        dataframe_valid_calendar = dataframe_valid_calendar.assign(
            idx=dataframe_valid_calendar.index
        )

        trips_to_check_if_cancel = dataframe_valid_calendar[["trip_id", "date", "idx"]]
        trips_to_check_if_cancel = pd.merge(
            trips_to_check_if_cancel, self.trips, on="trip_id"
        )
        trips_canceled = pd.merge(
            trips_to_check_if_cancel, self.calendar_dates, on=["service_id", "date"]
        )
        dataframe_valid_dates = dataframe_valid_calendar[
            ~dataframe_valid_calendar["idx"].isin(trips_canceled["idx"])
        ]
        dataframe_valid_dates = dataframe_valid_dates.assign(
            dep_time=dataframe_valid_dates["date"]
            + dataframe_valid_dates["departure_time_x"],
