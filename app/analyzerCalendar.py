...
class AnalyzerCalendar(Analyzer):
    ...
    def __init__(self, transport_type="FLIXBUS"):
        ...
        self.stop_times = pd.read_csv(os.path.join("Data", transport_type, "stop_times.txt"), low_memory=False)[
        ...
        if "stop_timezone" in stops.columns:
            ...
            self.stops = stops[["stop_id", "stop_name", "stop_lat", "stop_lon"]].assign(stop_timezone=self.timezone)
    ...
    def find_nearby_stops(self, coords: Coords) -> pd.DataFrame:
        return self.stops[
    ...
    def find_destinations_from_location(self, coords: Coords, date_min: datetime, date_max: datetime) -> pd.DataFrame:
        trip_ids_within_period: pd.Series = self.filter_trips_within_period(coords, date_min, date_max)
    ...
    def dates_from_trips(self, trips: pd.DataFrame, start_date: datetime, end_date: datetime) -> pd.DataFrame:
        ...
        for _, row in services_within_period.iterrows():
            ...
            df = df[
                df.apply(
                    lambda x: (True if x.iloc[monday_index + x.date.weekday()] == 1 else False),
                    axis=1,
                )
            ]
        ...
        dataframe_valid_dates = dataframe_valid_dates.assign(
            dep_time=dataframe_valid_dates["date"] + dataframe_valid_dates["departure_time_x"],
            arr_time=dataframe_valid_dates["date"] + dataframe_valid_dates["departure_time_y"],
        )
        dataframe_valid_dates["dep_time"] = pd.to_datetime(dataframe_valid_dates["dep_time"]).dt.tz_localize(
            pytz.timezone(self.timezone)
        )
        dataframe_valid_dates["arr_time"] = pd.to_datetime(dataframe_valid_dates["arr_time"]).dt.tz_localize(
            pytz.timezone(self.timezone)
        )
