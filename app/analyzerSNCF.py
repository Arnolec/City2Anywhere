...
class AnalyzerCalendarDatesSNCF(Analyzer):
    ...
    def __init__(self, transport_type="TER"):
        ...
        self.stop_times = pd.read_csv(
            os.path.join("Data", transport_type, "stop_times.txt")
    ...
    def find_nearby_stops(
        ...
        return self.stops_id[
            np.sqrt(
                (self.stops_id["stop_lat"] - lat) ** 2
                + (self.stops_id["stop_lon"] - lon) ** 2
            )
            < 1.05 * max_distance
        ]
    ...
    def get_trips_nearby_location(
        ...
        self.unique_departures = trips_containing_departure.drop_duplicates(
            subset="trip_id"
        )
    ...
    def filter_trips_within_period(
        ...
        services_within_period: pd.DataFrame = relevant_services[
            (relevant_services["date"] >= start_date)
            & (relevant_services["date"] <= end_date)
        ]
    ...
    def find_destinations_from_location(
        ...
        duplicate_destinations_stop_points: pd.DataFrame = (
            cities_after_inital_departure[
        ...
        duplicate_destinations_stop_area: pd.DataFrame = self.stops_area[
            self.stops_area["stop_id"].isin(
                duplicate_destinations_stop_points["parent_station"]
            )
        ]
        destinations_stop_area = duplicate_destinations_stop_area.drop_duplicates(
            subset="stop_id"
        )
    ...
    def find_trips_between_locations(
        ...
        trips_containing_departure: pd.DataFrame = pd.merge(
            self.stop_times, departure_stops, on="stop_id"
        )
        ...
        trips_containing_arrival: pd.DataFrame = pd.merge(
            self.stop_times, arrival_stops, on="stop_id"
        )
        ...
        trips: pd.DataFrame = pd.merge(
            trips_in_right_direction, self.trips, on="trip_id"
        )
        trips_and_calendar_dates: pd.DataFrame = pd.merge(
            trips, self.calendar_dates, on="service_id"
        )
    ...
    def get_list_of_cities(self) -> pd.DataFrame:
        ...
        appeareance_stop_area = appearance_stop_id.groupby("parent_station").sum()[
            "trip_id"
        ]
        df_stop_area = self.stops_area[
