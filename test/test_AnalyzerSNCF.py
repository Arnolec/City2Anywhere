...
def analyzer() -> Analyzer:
    return Analyzer("TEST/AnalyzerSNCF")
...
def test_find_trips_stops_not_found(analyzer) -> None:
    trips = analyzer.find_trips_between_locations(0, 0, 0, 0, datetime(2024, 7, 1), datetime(2024, 9, 1), pd.Timedelta("08:00:00"))
    assert trips.shape[0] == 0

def test_find_trips_hours_too_late(analyzer) -> None:
    trips = analyzer.find_trips_between_locations(50.0, 10.0, 48.0, 5.0, datetime(2024, 7, 1), datetime(2024, 7, 1), pd.Timedelta("20:00:00"))
    assert trips.shape[0] == 0

def test_find_trips_one(analyzer) -> None:
    trips = analyzer.find_trips_between_locations(50.0, 10.0, 48.0, 5.0, datetime(2024, 7, 1), datetime(2024, 7, 1), pd.Timedelta("08:00:00"))
    assert trips.shape[0] == 1
...
def test_find_trips_multiple(analyzer) -> None:
    trips = analyzer.find_trips_between_locations(100.0, 15.0, 48.0, 5.0, datetime(2024, 7, 1), datetime(2024, 8, 1), pd.Timedelta("08:00:00"))
